<!-- SEO Meta -->
<!--
  Title: Panth EU Withdrawal Button - Directive (EU) 2023/2673 Cancellation Function for Magento 2 | Panth Infotech
  Description: Panth EU Withdrawal Button adds the clear, accessible digital withdrawal (cancellation) function required by Directive (EU) 2023/2673 from 19 June 2026. Guest-capable two-step flow, durable-medium email proof, signed withdrawal link in order emails, configurable 14-day window, customer account history, admin request management, batch refund reminders. Hyva and Luma. Magento 2.4.4 - 2.4.9, PHP 8.1 - 8.4.
  Keywords: magento 2 eu withdrawal button, directive eu 2023/2673, magento 2 right of withdrawal, magento 2 order cancellation, magento 2 consumer rights, withdrawal button magento, hyva withdrawal button, luma withdrawal button
  Author: Kishan Savaliya (Panth Infotech)
  Canonical: https://github.com/mage2sk/module-eu-withdrawal
-->
...
[![Packagist](https://img.shields.io/badge/Packagist-mage2kishan%2Fmodule--eu--withdrawal-orange?logo=packagist&logoColor=white)](https://packagist.org/packages/mage2kishan/module-eu-withdrawal)
[![GitHub](https://img.shields.io/badge/GitHub-mage2sk%2Fmodule--eu--withdrawal-181717?logo=github&logoColor=white)](https://github.com/mage2sk/module-eu-withdrawal)
...
- [The Customer Flow](#the-customer-flow)
...
- [Hyvä and Luma](#hyvä-and-luma)
- [Batch Processing and Reminders](#batch-processing-and-reminders)
...
1. Show a **clearly labelled** withdrawal button that is **easy to find** and stays reachable for the whole withdrawal period, on desktop and mobile.
2. Let consumers cancel **without logging in** — guest orders included.
3. Use a **two-step process**: first a short form asking only for the data needed, then a confirmation screen before the cancellation is final.
4. Ask for **minimal data only**. If a reason is requested, it must be optional.
5. Send an **automatic confirmation on a durable medium** (such as email) containing the date, time and content of the withdrawal.
6. Include a **direct link to the cancellation function in every order confirmation email**.
7. Keep consumers informed about **exclusions, return shipping costs and refunds** before purchase.
...
- **Pop-up modal** — clicking any entry point opens the form in place; without JavaScript it falls back to the dedicated `/withdrawal` page.
...
1. The customer opens the form from the floating tab, the link in an order email, or the order view page.
2. **Step 1** — they enter order number, email and name (logged-in customers pick the order from a list). The reason is optional.
3. The store checks that the order matches the email and is still inside the withdrawal period.
4. **Step 2** — a summary of the order is shown. The customer clicks **Confirm withdrawal**.
5. The withdrawal is recorded, a confirmation email is sent as durable proof, staff are notified, and a comment is added to the order.
6. The customer sees a **proof page** with a reference number.
...
1. Download the latest release ZIP from [Packagist](https://packagist.org/packages/mage2kishan/module-eu-withdrawal) or the [Adobe Commerce Marketplace](https://commercemarketplace.adobe.com).
2. Extract the contents to `app/code/Panth/EuWithdrawal/` in your Magento installation.
...
<p align="center">
  <strong>Get your store ready for 19 June 2026.</strong><br/>
  <a href="https://kishansavaliya.com/get-quote">
